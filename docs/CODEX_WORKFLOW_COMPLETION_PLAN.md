...
- PR activo de revisión: [#277](https://github.com/jdlc86/gestionpisos/pull/277). Sustituye a #276 únicamente para re-disparar CI sobre el HEAD corregido; #276 queda cerrado sin merge.
...
- Correcciones de revisión independiente aplicadas por ChatGPT: los `occupancy.created` obsoletos por lifecycle se retiran como `processed_with_errors` sin bloquear la cola, preservando reintento para fallos recuperables; `reject` mantiene tarea y ejecución en `rejected`, recuperando Historial y `workflow_rejected`. Regresiones específicas añadidas para ambos casos.
...
- Siguiente acción exacta: comprobar los tres checks del HEAD final de PR #277, revisar hilos y diff final, fusionar solo si todo está verde, verificar despliegue post-merge y ejecutar E2E humano antes de marcar WF-04 `VERIFIED`. Solo ChatGPT puede autorizar WF-05.
