...
-- Include pending staff in permission management without granting them any role.
...
-- Deactivation also accepts a pending onboarding identity. This lets an admin revoke
-- an invitation without first activating the account.
...
-- Narrow one-off backfill for the two test employees created by the old no-password
-- flow. The time window is fixed historical evidence, not a generated-id list.
