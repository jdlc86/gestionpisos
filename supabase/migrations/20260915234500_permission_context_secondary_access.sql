-- Expose active secondary property access to the existing permission-management context.
...
as $$
declare v_actor uuid:=auth.uid(); v_org uuid:=p_organization_id; v_is_root boolean:=false; v_is_admin boolean:=false; v_result jsonb;
begin
 ...
 if v_org is null then
   select organization_id into v_org from public.user_roles where user_id=v_actor and organization_id is not null and revoked_at is null order by case when role='admin' then 0 else 1 end,created_at limit 1;
 end if;
 ...
 select jsonb_build_object(
  ...
  'people',coalesce((select jsonb_agg(x order by x->>'display_name',x->>'email') from (
    ...
    from (select ur.user_id,jsonb_agg(ur.role::text order by ur.role::text) roles from public.user_roles ur where ur.organization_id=v_org and ur.revoked_at is null group by ur.user_id) u
    left join public.profiles p on p.user_id=u.user_id) q),'[]'::jsonb),
  ...
  from public.properties_v2 pr left join public.property_staff_access_v3 sa on sa.property_id=pr.id and sa.assignment_type='responsible' and sa.revoked_at is null
  where pr.organization_id=v_org and pr.archived_at is null),'[]'::jsonb),
  'capability_holders',coalesce((select jsonb_agg(jsonb_build_object('id',h.id,'capability',h.capability,'holder_user_id',h.holder_user_id,'granted_at',h.granted_at) order by h.capability) from public.admin_capability_holders h where h.organization_id=v_org and h.revoked_at is null),'[]'::jsonb),
  'pending_requests',coalesce((select jsonb_agg(jsonb_build_object('id',r.id,'capability',r.capability,'requester_user_id',r.requester_user_id,'requested_at',r.requested_at) order by r.requested_at) from public.admin_capability_requests r where r.organization_id=v_org and r.status='pending'),'[]'::jsonb)
 ) into v_result;
 return v_result;
end $$;
