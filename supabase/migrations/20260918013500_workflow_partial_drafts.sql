-- GestionPisos · Flujos de Trabajo · borradores parciales y decisiones explícitas
-- Riesgo R3: cambia la semántica de autoría de definiciones persistentes.
-- Un borrador puede guardarse incompleto. Ninguna opción de negocio se presupone.
...
alter table public.workflow_definitions_v2
  ...
  alter column assignment_type drop not null,
  alter column close_type drop not null;

alter table public.workflow_definitions_v2
  add column if not exists authoring_complete boolean not null default false;
...
as $$
  select coalesce(
    jsonb_typeof(p_spec) = 'object'
    and case
      when coalesce(p_spec ->> 'authoringVersion','') ~ '^[0-9]+$'
        then (p_spec ->> 'authoringVersion')::integer >= 2
      else false
    end
    and char_length(btrim(coalesce(p_spec ->> 'flowName',''))) between 3 and 80
    and (p_spec ->> 'flowType') in ('cleaning','inspection','maintenance','checkin','checkout','custom')
    and (p_spec ->> 'scopeType') in ('organization','property','room','occupancy')
    and (p_spec ->> 'triggerType') in ('manual','recurring','scheduled_once','event')
    and (
      (p_spec ->> 'triggerType') <> 'recurring'
      or (p_spec ->> 'recurrence') in ('weekly','biweekly','monthly','custom')
    )
    and (p_spec ->> 'assignmentType') in ('property_responsible','active_occupants_rotation','fixed_person','role','manual')
    and (
    ...
    and (p_spec ->> 'closeType') in ('auto','human_review','domain_adapter'),
    false
  );
$$;
...
-- Los valores escritos por la UI anterior eran defaults implícitos. No se muestran
-- como decisiones reales hasta que el borrador sea revisado con authoringVersion >= 2.
update public.workflow_definitions_v2
set flow_type = null,
    scope_type = null,
    trigger_type = null,
...
where status = 'draft'
  and not (
    case
      when coalesce(draft_spec ->> 'authoringVersion','') ~ '^[0-9]+$'
        then (draft_spec ->> 'authoringVersion')::integer >= 2
      else false
    end
  );
...
comment on column public.workflow_definitions_v2.authoring_complete is
  'True only when every required creator section contains an explicit v2 authoring decision. It does not by itself make the definition publishable.';
