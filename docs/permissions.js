...
const canCreate=data?.actor?.is_root||writeHolder?.holder_user_id===me;if($("createUserForm"))$("createUserForm").hidden=!canCreate;if($("createUserNote")&&!canCreate)$("createUserNote").textContent="Solo ROOT o el administrador titular del control administrativo puede crear usuarios.";
$("adminsList").innerHTML=admins.length?admins.map(p=>card(personName(p),p.email,p.user_id===writeHolder?.holder_user_id?"Control administrativo":"Admin")).join(""):card("Sin administradores","No hay administradores activos.");
