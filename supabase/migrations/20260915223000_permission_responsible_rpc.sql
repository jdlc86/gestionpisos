-- Preserve capability-holder history while keeping the partial unique index as the current-holder invariant.
alter table public.admin_capability_holders
  drop constraint if exists admin_capability_holders_organization_id_capability_key;
...
begin
  ...
  if v_org is null then
    raise exception 'property_not_found_or_archived';
  end if;
  ...
  if not exists (
    ...
    if not exists (
      ...
      where organization_id = v_org
        and capability = 'property_lifecycle'
        and holder_user_id = v_actor
        and revoked_at is null
    ) then
      raise exception 'property_lifecycle_required';
    end if;
  end if;
  ...
  select id, employee_user_id
    into v_assignment_id, v_previous_user
  ...
  if v_assignment_id is not null and v_previous_user = p_employee_user_id then
  ...
  insert into public.audit_log_v2 (
    ...
    jsonb_build_object(
      ...
      'previous_responsible_user_id', v_previous_user
    )
  );
